...
@interface DAVCarouselAdView()<UIScrollViewDelegate>
...
@implementation DAVCarouselAdView


#pragma mark- 初始化方法
//创建用来缓存图片的文件夹
+ (void)initialize {
    NSString *cache = [[NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) lastObject] stringByAppendingPathComponent:@"DAVCarousel"];
    BOOL isDir = NO;
...
- (UIPageControl *)pageControl {
    if (!_pageControl) {
        ...
        self.pageControl.currentPageIndicatorTintColor = DAV_COLOR_16(FF5400);
...
- (void)setImageArray:(NSArray *)imageArray{
    ...
    for (int i = 0; i < imageArray.count; i++) {
        if ([imageArray[i] isKindOfClass:[UIImage class]]) {
            ...
            [_images addObject:[UIImage imageNamed:@"ad_placeholder.png"]];
